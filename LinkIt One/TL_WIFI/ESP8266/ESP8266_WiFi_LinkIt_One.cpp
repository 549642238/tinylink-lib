...
*/
bool ESP8266_MQTT::connect(char* serverName, int port, char* clientName, char* userName, char* password){
	_port = port;
...
/*
int ESP8266_MQTT::publish(const String& topicName, const String& data){
	char* _topicName = (char*)malloc(sizeof(char)*(topicName.length()+1));
	topicName.toCharArray(_topicName, topicName.length()+1);
	char* _data = (char*)malloc(sizeof(char)*(data.length()+1));
	data.toCharArray(_data, data.length()+1);
	int res = publish(_topicName, _data);
	free(_topicName);
	free(_data);
	return res;
}
*/
bool ESP8266_MQTT::publish(const char* topicName, char* data, int qos){
	if(!reconnect()){
		return false;
	}
...
*/
bool ESP8266_MQTT::subscribe(const char* topicFilter, MQTT::Client<IPStack, Countdown>::messageHandler mh, int qos){
	if(!reconnect()){
		return false;
	}
	MQTT::QoS Qos;
	...
	return (client->subscribe(topicFilter, Qos, mh)==0);
}

bool ESP8266_MQTT::unsubscribe(const char* topicFilter){
	return (client->unsubscribe(topicFilter)==0);
}
