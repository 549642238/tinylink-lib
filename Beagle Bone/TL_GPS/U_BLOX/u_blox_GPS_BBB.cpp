...
int u_blox::parse(char *gpsData){
	...
	if(gpsData[0] == '$')
	{
		...
		_data.TIME.hour     = (gpsData[tmp + 0] - '0') * 10 + (gpsData[tmp + 1] - '0');
		_data.TIME.minute   = (gpsData[tmp + 2] - '0') * 10 + (gpsData[tmp + 3] - '0');
		_data.TIME.second   = (gpsData[tmp + 4] - '0') * 10 + (gpsData[tmp + 5] - '0');

		tmp = getComma(9, gpsData);
		_data.TIME.day     = (gpsData[tmp+0]-'0')*10+(gpsData[tmp+1]-'0');  
		_data.TIME.month   = (gpsData[tmp+2]-'0')*10+(gpsData[tmp+3]-'0');  
		_data.TIME.year    = (gpsData[tmp+4]-'0')*10+(gpsData[tmp+5]-'0')+2000; 	
		...
		if (checksum == 'A')
		{
			if (latitude != 0 && longitude != 0)
			{
				...
				#ifdef DEBUG
				cout << "GPS  reading success Time:"<<" "<<_data.TIME.year << "/"<<_data.TIME.month<<"/"<<_data.TIME.day <<" "<<_data.TIME.hour<< ":"<< _data.TIME.minute<<":"<<_data.TIME.second<<endl;
