#ifndef TINYLINK_URLUTILS
#define TINYLINK_URLUTILS
...
public:
    UrlUtils(String buf, const String url, const String method, Stream *cerr = NULL) {
        _data = escape(buf);
        decode_url(url);
    ...
    String get_request() {
        ...
        if (_method == String("POST")) {
            ans = "POST " + _uri + " HTTP/1.1" + term +
                ...
                "Content-Length: " + String(_data.length(), DEC) + term +
                term + 
                _data;
